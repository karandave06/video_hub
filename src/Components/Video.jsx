...
const Video = () => {
  const [videourl,setvideourl] = useState(videosArr[0])
  return (
    <Stack direction={[ 'column', 'row']} h={'100vh'}>
      ...
      <VStack p={'8'} spacing={'8'} w={['full', 'xl']} alignItems={'stretch'}>
      {
        videosArr.map((item,index)=>(
          <Button key={index} variant={'ghost'} colorScheme="purple" onClick={()=>setvideourl(item)}>
      Lacture {index+1}
      </Button>
        ))
      }
